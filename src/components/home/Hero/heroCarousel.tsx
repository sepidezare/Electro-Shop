  ...
  const defaultSlides: CarouselSlide[] = [
    {
      ...
      description: 'Get ready for summer with our exclusive collection. Fresh styles, vibrant colors, and comfortable fabrics.',
      ctaText: 'Shop Now',
    ...
    },
    {
      ...
      description: 'Don\'t miss our biggest sale of the year. Limited time offers on selected items.',
        ...
        {displaySlides.map((slide, index) => (
          ...
          >
            ...
            <div className="relative z-10 flex h-full items-center">
              <div className={`container mx-auto px-6 ${showThumbnails ? 'pr-12' : ''}`}>
                <div className={`text-white ${showThumbnails ? 'max-w-2xl' : 'max-w-lg'}`}>
                  {slide.badge && (
                  ...
                  <h1 className={`mb-4 font-bold leading-tight ${
                    showThumbnails ? 'text-5xl md:text-6xl' : 'text-4xl md:text-5xl'
                  }`}>
                    {slide.title}
                  </h1>
                  
                  <h2 className={`mb-4 font-light text-gray-200 ${
                    showThumbnails ? 'text-2xl md:text-3xl' : 'text-xl md:text-2xl'
                  }`}>
                    {slide.subtitle}
                  </h2>
                  
                  <p className={`mb-8 text-gray-300 leading-relaxed ${
                    showThumbnails ? 'text-lg md:text-xl' : 'text-base md:text-lg'
                  }`}>
                    {slide.description}
                  </p>
